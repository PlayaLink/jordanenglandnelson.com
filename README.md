...
- `/` is now generated by `src/pages/index.astro`.
- `/benchmarkr` is an Astro-native product-design case study.

The Benchmarkr case study uses intentionally unfinished CSS wireframes for its
visual slots. They document the expected product views without presenting
invented shipped screenshots and can be replaced with publishable product
captures later.
